...
export const crearJuego = async (req, res) => {
    try {
        const { nombre, descripcion, fecha, exposicion_id, estado_id, piezas } = req.body;
        console.log(req.body)

        // Verificar si la exposición asociada al juego existe
        const exposicion = await Exposicion.findByPk(exposicion_id);
        if (!exposicion) {
            return res.status(404).json({ error: "La exposición asociada no existe" });
        }
