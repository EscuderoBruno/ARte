...
export const updatePiezaById = async (req, res) => {
  try {
    const { sala_id, estado_id, imagen_delete, pictograma_delete, modelo_delete } = req.body;
    ...
    if(nuevaPieza.imagen === null && piece.imagen !== null && fs.existsSync(`./uploads/${piece.imagen}`)){
      fs.unlinkSync(`./uploads/${piece.imagen}`);
    }
    if(nuevaPieza.pictograma === null && piece.pictograma !== null && fs.existsSync(`./uploads/${piece.pictograma}`)){
      fs.unlinkSync(`./uploads/${piece.pictograma}`);
    }
    if(nuevaPieza.modelo === null && piece.modelo !== null && fs.existsSync(`./uploads/${piece.modelo}`)){
...
export const deletePiezaById = async (req, res) => {
  try {
    ...
    if(pieza.imagen !== null && fs.existsSync(`./uploads/${pieza.imagen}`)){
      fs.unlinkSync(`./uploads/${pieza.imagen}`);
    }
    if(pieza.pictograma !== null && fs.existsSync(`./uploads/${pieza.pictograma}`)){
      fs.unlinkSync(`./uploads/${pieza.pictograma}`);
    }
    if(pieza.modelo !== null && fs.existsSync(`./uploads/${pieza.modelo}`)){
...
export const getInfoPieza = async (req, res) => {
  try {
    ...
    if (!info) {
      return res.status(404).json({ error: "Informacion de pieza no encontrada" });
    }
    res.status(200).json(info);
  } catch (error) {
    console.log(error);
    return res.status(500).json({ error: "Error al cargar la informacion de pieza" });
  }
}
...
export const obtenerTotalYDiferenciaPiezasMeses = async (req, res) => {
  try {
      ...
      const primerDiaMesActual = new Date(fechaActual.getFullYear(), mesActual - 1, 1); // Se resta 1 porque los meses son indexados desde 0

      // Calcular el primer día del mes pasado
      let primerDiaMesPasado;
      if (mesActual === 1) { // Si el mes actual es enero, el mes pasado es diciembre del año anterior
        primerDiaMesPasado = new Date(fechaActual.getFullYear() - 1, 11, 1);
      } else {
        primerDiaMesPasado = new Date(fechaActual.getFullYear(), mesActual - 2, 1); // Resta 2 para obtener el mes pasado
      }
      ...
      res.status(200).json({ total: piezasMesActual, diferencia: diferenciaTotalPiezas });
  } catch (error) {
      console.error('Error al obtener el total y la diferencia en el total de piezas entre meses:', error);
