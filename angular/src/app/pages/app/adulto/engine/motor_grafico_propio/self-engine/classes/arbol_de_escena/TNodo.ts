...
export class TNodo {
  ...
  public setEntidad(entidad_del_nodo: TEntidad): boolean {
    if(this.entidad != undefined) { console.log("Warning: Estás cambiando la entidad");};
  ...
  public recorrer(matriz_acumulada: mat4, actualizar_matriz_forzado_por_padre: boolean): void {
    ...
    //console.log("RECORRIDO: " + mat4.str(this.matrizTransf));
    if(this.entidad != undefined) {
      this.entidad.dibujar();
    }
    if(this.hijos) {
      for(let i in this.hijos) {
        this.hijos[i].recorrer(this.matrizTransf, actualizar_matriz_forzado_por_padre);
      }
    } else {
  ...
  public calcularRotacion(): mat4 {
    var matriz_rotada_X = mat4.create(),
      ...
      /*
      for (let i in matriz_rotada_XYZ) {
        //matriz_rotada_XYZ[i] = Math.round(matriz_rotada_XYZ[i]);
        var number = matriz_rotada_XYZ[i];
        var rounded = Math.round(number * 10) / 10;
        var fixed = rounded.toFixed(1);
        parseFloat(number.toFixed(2));
        matriz_rotada_XYZ[i] = number;
      }
      */
  ...
  public getTraslacion(): vec3 { return this.traslacion; }
  public getRotacion(): vec3 { return this.rotacion; }
  public getEscalado(): vec3 { return this.escalado; }
