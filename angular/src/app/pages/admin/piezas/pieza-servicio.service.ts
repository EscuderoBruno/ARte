...
export class PieceService {
  ...
  getLista(idioma: string, filtrosValues?: any): Observable<PiezaListaAPI> {
    ...
    if(filtrosValues){
      this.options.params = this.options.params.set('nombre', filtrosValues?.nombre?.value).set('sala', filtrosValues?.sala?.value).set('pag', filtrosValues?.pagina?.value).set('lim', filtrosValues?.numero?.value);
    }
  ...
  getInfo(id: string, idioma: string): Observable<Informacion> {
    return this.http.get<any>(`${this.apiUrl}/info/${id}/${idioma}`, this.options);
  }
