...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterLink, PiezasSelectorComponent],
...
export class EditarJuegoComponent {
  ...
  ngOnInit(): void {
    ...
    this.juegoService.getOne(this.id).subscribe(data => {
      ...
      this.editarJuego.setValue({
        ...
        fecha:  this.dateService.transformarFecha(new Date(data.fecha || '') || new Date(), 'html'),
        exposicion: data.exposicion_id || '0',
        estado: data.estado_id || '0'
      })
      data.Pregunta.forEach((pregunta: Pregunta) => {
        const index = pregunta.pieza.informacion.findIndex(_info => _info.idioma_id === 'es')
        if(index === - 1) return;
        this.piezas.push({
          ...
          texto_completo: pregunta.pieza.informacion[index].texto_completo || '',
    ...
    this.breadcrumbService.updateBreadcrumb('Editar juego', ['Home', 'Juego', 'Editar']);
  }
  ...
  handlerSubmit(evt: Event){
    ...
    if(!this.editarJuego.valid){ return; }
